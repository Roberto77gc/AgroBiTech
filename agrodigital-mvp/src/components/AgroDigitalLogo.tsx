...
interface AgroDigitalLogoProps {
	size?: 'sm' | 'md' | 'lg' | 'xl'
	className?: string
	showText?: boolean
}

const AgroDigitalLogo: React.FC<AgroDigitalLogoProps> = ({ 
	size = 'md', 
	className = '',
	showText = true 
}) => {
		...
		<div className={`flex items-center space-x-3 ${className}`}>
			...
			{showText && (
				<div className="flex flex-col">
					<h1 className={`${textSizes[size]} font-black bg-gradient-to-r from-emerald-600 via-green-600 to-teal-700 bg-clip-text text-transparent leading-none`}>
						AgroDigital
					</h1>
					<p className="text-xs text-gray-600 font-medium leading-none">
						La Revolución Agrícola del Siglo XXI
					</p>
				</div>
			)}
		</div>
...
export default AgroDigitalLogo
